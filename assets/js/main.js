...
document.addEventListener("DOMContentLoaded", function () {
    const texts = ["Frontend Developer", "UI/UX Designer", "AI/ML Enthusiast"];
    let count = 0;
    let index = 0;
    let currentText = "";
    let letter = "";
    ...
    const delay = 2000; // Delay between words
    ...
    function type() {
        ...
        if (letter.length === currentText.length) {
            setTimeout(erase, delay); // Wait before starting the erasing effect
        } else {
            setTimeout(type, speed); // Typing speed
        }
    }
    ...
    function erase() {
        ...
        if (letter.length === 0) {
            count++;
            index = 0;
            setTimeout(type, speed); // Start typing the next word
        } else {
            setTimeout(erase, eraseSpeed); // Erasing speed
        }
    }
